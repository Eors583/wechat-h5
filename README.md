...
服务器需要安装 Docker Engine 和 Docker Compose。没有域名时，可以先使用公网 IP 和独立端口部署，例如开放 TCP 3210 端口后执行：

```bash
git clone https://github.com/Eors583/wechat-h5.git
cd wechat-h5
cp deploy.env.example .env
# 编辑 .env，填写 PUBLIC_BASE_URL、APP_PORT 和 WEB3FORMS_ACCESS_KEY
docker compose up -d --build
```

使用示例配置时，访问地址是 `http://服务器IP:3210`。SQLite 数据保存在 `lead_data` 卷中，重新构建容器不会删除数据。IP + HTTP 没有传输加密，只适合临时或无法使用域名的场景。

如果以后有域名，把 `BIND_ADDRESS` 改为 `127.0.0.1`，填写 `DOMAIN`，并启用可选的 Caddy HTTPS 配置：
...
健康检查地址为 `http://服务器IP:3210/api/health`；启用域名和 HTTPS 后则为 `https://你的域名/api/health`。正常时返回 `{"ok":true}`。

## 挂到微信公众号文章

1. 先把站点部署成任何人都能访问的地址；建议使用 HTTPS，没有域名时也可以临时使用 IP + 端口。
2. 在公众号图文编辑页开启“原文链接”。
3. 粘贴带渠道参数的地址，例如：

```text
http://47.99.126.8:3210/?utm_source=my_official_account&utm_medium=wechat_article&utm_campaign=202609
```
